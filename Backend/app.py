...
def getPassword(website = None, userId = None):
    ...
    try:
        ...
        # private_pem = str.encode(user.secret1)
        # private_key = serialization.load_pem_private_key(private_pem, password=str.encode(user.password))
        ...
        cipher_list = password
        cipher_list[3] = str_plaintext #rsa_cipher_text_bytes.hex()

        print(cipher_list)

        response = jsonify({"Password" : cipher_list})
...
def addPassword():
    ...
    try:
        ...
        public_pem = str.encode(user.public_key)
...
def editPassword():
    ...
    try:
        sql_statement = "update passwords set password = '{}', decription =  '{}', nonce = '{}'  where user_id = {} and website = '{}' and username = '{}'"
